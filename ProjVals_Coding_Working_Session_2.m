...
while convCheck >= 10
    ...
    Ca2 = Ca;
    W2 = sqrt(Wu2^2 + Wa2^2);
    alpha2p = atan(Wa2/Wu2);
    
    % --------------------- Rotor Exit Kinematics ------------------------ %
    
    % Starting from Lecture 4 Eqn 26
    Wa3 = Ca; Ca3 = Ca;
    Wu3 = -U; W3 = sqrt(Wa3^2 + Wu3^2);
    C3 = Ca; Cu3 = Wu3 + U;
    alpha3p = atan(Wa3/Wu3);
    ...
    Pw3 = Kloss_R_guess * Pw2;
    P3ver = Pw3/(1 + ((gamma-1)*Mw3^2)/2)^(gamma/(gamma-1));
    rho2 = P2/(Rgas*T2);
    rho3 = P3/(Rgas*T3);
    
    % --------------------- Geometry ------------------------ %
    A2 = mdot/(rho2*Ca2); A3 = mdot/(rho3*Ca3);
    L2 = A2/(2*pi*rmean*BL); 
    L3 = A3/(2*pi*rmean*BL);
    L2D = L2/(rmean*2); 
    L3D = L3/(rmean*2);
...
% ----------------- stator exit kinematics ---------------------- %
r2tip=rmean+(L2/2);
U2tip=pi*r2tip*Nrpm/30;
Ca2tip = Ca;

Cu2tip = rmean*Cu2/r2tip;
Wu2tip = Cu2tip-U2tip;
Wa2tip = Ca;
W2tip = sqrt((Wu2tip)^2+(Wa2tip)^2);
alpha2tip = atand(Ca/Cu2tip);
alpha2ptip = atand(Wa2tip/Wu2tip);
C2tip = Ca/sind(alpha2tip);

% ----------------- rotor exit kinematics ---------------------- %
r3tip=rmean+(L3/2);
U3tip=(pi*r3tip*Nrpm)/30;
alpha3tip = nozInAng;

Wa3tip = Ca;
Wu3tip = -U3tip;
W3tip = sqrt(Wa3tip^2+Wu3tip^2);
Cu3tip = (rmean*Cu3)/r3tip;
Ca3tip = Ca;
C3tip = Ca;
alpha3ptip = atand(Wa3tip/Wu3tip);

% ----------------- stator exit kinematics ---------------------- %
r2hub = rmean-(L2/2);
U2hub = (pi*r2hub*Nrpm)/30;
Ca2hub = Ca;

Cu2hub = rmean*Cu2/r2hub;
Wu2hub = Cu2hub-U2hub;
Wa2hub = Ca;
W2hub = sqrt(Wu2hub^2+Wa2hub^2);
alpha2hub = atand(Ca2hub/Cu2hub);
alpha2phub = atand(Wa2hub/Wu2hub);
C2hub = Ca/sind(alpha2hub);

% ----------------- rotor exit kinematics ---------------------- %
r3hub = rmean-(L3/2);
U3hub = (pi*r3hub*Nrpm)/30;
alpha3hub = nozInAng;

Wa3hub = Ca;
Wu3hub = -U3hub;
W3hub = sqrt(Wu3hub^2+Wa3hub^2);
Ca3hub = Ca;
Cu3hub = 0; % From Lecture 4 Section 9.2.1...Cu3=0 b/c dischange flow is axial so therefore there is no swirl at station 3
C3hub = Ca;
alpha3phub = atand(Wa3hub/Wu3hub);

% ----------------- Throat gap openings ---------------------- %
% ----------STATOR -------------------------------- %
% From Lecture 4 eqn 64 and in Section 9.2.3
o2 = sin(abs(alpha2))* sstator * BL; % alpha2 is in rad
    % Check: o2 = (o2tip + o2hub)/2...if not, then need to refine design b/c
    % r2tip and r2hub are only approximate throat sizes...From Lecture 4
    % Section 9.2.3
o2tip = sind(abs(alpha2tip)) * 0.0364 * BL; % sstatortip = r2tip = 0.4923 --> o2tip = 0.1851: From Lecture 4 eqn 68
o2hub = sind(abs(alpha2hub)) * 0.0284 * BL; % sstatorhub = r2hub = 0.3839 --> o2hub = 0.1166: From Lecture 4 eqn 68

% ----------ROTOR -------------------------------- %
% From Lecture 4 eqn 65 and in Section 9.2.3
o3 = sin(abs(alpha3p)) * srot * BL; % alpha3p is in rad
    % Check: o3 = (o3tip + o3hub)/2...if not, then need to refine design b/c 
    % r3tip and r3hub are only approximate throat sizes...From Lecture 4
    % Section 9.2.3
o3tip = sind(abs(alpha3ptip)) * 0.0388 * BL; % sstatortip = r3tip = 0.4938 --> o3tip = 0.1648: From Lecture 4 eqn 69
o3hub = sind(abs(alpha3phub)) * 0.03005 * BL; % sstatorhub = r3hub = 0.3825 --> o3hub = 0.1577: From Lecture 4 eqn 69

% --3D Free Vortex thermodynamic quantities ------- %
% ----------------- stator exit tip ---------------------- %
% From Lecture 4 in Sections 9.2.1 and 9.2.2
T2tip = T02-(C2tip^2/(2*cp));
a2tip = sqrt(gamma*Rgas*T2tip);
M2tip = C2tip/a2tip;
P2tip = P02/(1+((gamma-1)*(M2tip^2))/2)^(gamma/(gamma-1));
Mwtip = W2tip/a2tip;
Pw2tip = P2tip*(1+((gamma-1)*(Mwtip^2))/2)^(gamma/(gamma-1));
Tw2tip = T2tip*(1+((gamma-1)*(Mwtip^2))/2);

% ----------------- stator exit hub ---------------------- %
% From Lecture 4 in Sections 9.2.1 and 9.2.2
T2hub = T02-(C2hub^2/(2*cp));
a2hub = sqrt(gamma*Rgas*T2hub);
M2hub = C2hub/a2hub;
P2hub = P02/(1+((gamma-1)*(M2hub^2))/2)^(gamma/(gamma-1));
Mwhub = W2hub/a2hub;
Pw2hub = P2hub*(1+((gamma-1)*(Mwhub^2))/2)^(gamma/(gamma-1));
Tw2hub = T2hub*(1+((gamma-1)*(Mwhub^2))/2); % couldn't find the eqn for this in Lecture 4 notes but value is correct

% ========== Performance ======================================== %
work = U*Cu2; % From lecture 5 eqn 32
ett_new = (1-(T03/T01))/(1-(P03/P01)^((gamma-1)/gamma)); % From Lecture 5 eqn 28
ett = ett_new;
effworktt = work / delh0is; % NEED TO CHECK should be 0.9105...From Lecture 4 eqn 17
ets = (1-(T03/T01))/(1-(P3/P01)^((gamma-1)/gamma)); % from Lecture 2 eqn 40
Power = work * massflow / 745.6999; % From Lecture 4 eqn 18...units in HP
Powerett = delh0is * ett * massflow / 745.6999; % NEED TO CHECK should be 931.3835...units in HP
